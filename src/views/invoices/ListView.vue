...
import InvoiceAction from './InvoiceAction.vue'
...
if (pageParam) {
  invoiceStore.page = Number(pageParam)
} else {
  invoiceStore.page = 1
}
...
const { paginatedInvoices, page, itemsPerPage, totalInvoices, bulkLoading, deletingIds } =
  storeToRefs(invoiceStore)

async function onApply() {
  await invoiceStore.handleBuilAction()
}
</script>

<template>
  <div>
    <PageTitle title="Invoice List" subtitle="View and manage all invoices.">
      <RouterLink to="/dashboard/invoice/add">
        <Button class="button-primary-outline">Add New Invoice</Button>
      </RouterLink>
    </PageTitle>
    ...
    <div class="flex flex-col items-start md:flex-row md:items-center gap-4 mb-2">
      <div class="flex items-center gap-2 w-full md:w-62">
        ...
        <Button :disabled="bulkLoading" variant="outline" @click.prevent="onApply" class="w-18.75">
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead class="min-w-35">Invoice#</TableHead>
            <TableHead class="min-w-35">Client</TableHead>
            <TableHead class="min-w-30">Amount (£)</TableHead>
            <TableHead class="min-w-50">Description</TableHead>
            <TableHead class="min-w-35">Due Date</TableHead>
            <TableHead class="min-w-30"> Status </TableHead>
            <TableHead class="w-35"> Paid At </TableHead>
          </TableRow>
        </TableHeader>
        <TransitionGroup tag="tbody" name="row">
          ...
          >
            ...
            <TableCell>
              <span v-if="paidAt" v-html="ukFormat(paidAt)"></span>

...
const store = useFundersStore()
const { confirm } = useConfirmDialog()

const handEditAction = (id) => {
  const url = `/dashboard/funder/edit/${id}`
  router.push(url)
}

const handDeleteAction = async (id) => {
  const confirmed = await confirm('Are you sure you want to move this funder to trash?', {
...
const handViewAction = (id) => {
  const url = `/dashboard/funder/show/${id}`
  router.push(url)
}
...
<template>
  <DropdownMenu>
    ...
    <DropdownMenuContent class="w-45 rounded">
      <DropdownMenuLabel class="text-xs opacity-75">Funder Action</DropdownMenuLabel>

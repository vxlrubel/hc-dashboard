...
import RotaAction from './RotaAction.vue'
...
if (pageParam) {
  rotaStore.page = Number(pageParam)
} else {
  rotaStore.page = 1
}
...
const { paginatedRotas, page, itemsPerPage, totalRotas, buikLoading, deletingIds } =
  storeToRefs(rotaStore)

async function onApply() {
  await rotaStore.handleBuilAction()
}
</script>

<template>
  <div>
    <PageTitle title="Rota List" subtitle="View and manage all rotas.">
      <RouterLink to="/dashboard/rota/add">
        <Button class="button-primary-outline">Add New Rota</Button>
      </RouterLink>
    </PageTitle>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead class="min-w-35">Employee</TableHead>
            <TableHead class="min-w-35">Shift Date</TableHead>
            <TableHead class="min-w-25">Start</TableHead>
            <TableHead class="min-w-25">End</TableHead>
            <TableHead class="min-w-30">Role</TableHead>

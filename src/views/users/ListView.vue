...
if (pageParam) {
  userStore.page = Number(pageParam)
} else {
  userStore.page = 1
}
...
const { paginatedUsers, page, itemsPerPage, totalUsers, buikLoading, deletingIds } =
  storeToRefs(userStore)

async function onApply() {
  await userStore.handleBuilAction()
}
</script>

<template>
  <div>
    <PageTitle title="User List" subtitle="View and manage all users.">
      <RouterLink to="/dashboard/user/add">
        <Button class="button-primary-outline">Add New User</Button>
      </RouterLink>
    </PageTitle>

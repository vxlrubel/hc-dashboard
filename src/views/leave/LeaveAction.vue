...
const handEditAction = (id) => {
  const url = `/dashboard/leave/edit/${id}`
  router.push(url)
}

const handDeleteAction = async (id) => {
  const confirmed = await confirm('Are you sure you want to move this leave to trash?', {
...
const handViewAction = (id) => {
  const url = `/dashboard/leave/show/${id}`
  router.push(url)
}
...
<template>
  <DropdownMenu>
    ...
    <DropdownMenuContent class="w-45 rounded">
      <DropdownMenuLabel class="text-xs opacity-75">Leave Action</DropdownMenuLabel>

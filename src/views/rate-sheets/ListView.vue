...
import RateSheetAction from './RateSheetAction.vue'
...
if (pageParam) {
  rateSheetStore.page = Number(pageParam)
} else {
  rateSheetStore.page = 1
}
...
const { paginatedRateSheets, page, itemsPerPage, totalRateSheets, buikLoading, deletingIds } =
  storeToRefs(rateSheetStore)

async function onApply() {
  await rateSheetStore.handleBuilAction()
}
</script>

<template>
  <div>
    <PageTitle title="Rate Sheet List" subtitle="View and manage all rate sheets.">
      <RouterLink to="/dashboard/rate-sheet/add">
        <Button class="button-primary-outline">Add New Rate Sheet</Button>
      </RouterLink>
    </PageTitle>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead class="min-w-20">Hourly Rate</TableHead>
            <TableHead class="min-w-20">Overnight Rate</TableHead>
            <TableHead class="min-w-20">Weekend Rate</TableHead>
            <TableHead class="min-w-30">Effective From</TableHead>
            <TableHead class="min-w-30">Effective To</TableHead>
        ...
        <TransitionGroup tag="tbody" name="row">
          ...
          >
            ...
            <TableCell>
              <RateSheetAction :id="id" />
            </TableCell>
            <TableCell>{{ name }}</TableCell>
            <TableCell>{{ description }}</TableCell>
            <TableCell>{{ hourlyRate }}</TableCell>
            <TableCell>{{ overnightRate }}</TableCell>
            <TableCell>{{ weekendRate }}</TableCell>
            <TableCell>{{ effectiveFrom }}</TableCell>
            <TableCell>{{ effectiveTo }}</TableCell>
            <TableCell>

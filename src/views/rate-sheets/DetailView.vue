...
<template>
  <div>
    <PageTitle title="Rate Sheet Details" subtitle="View and manage rate sheet information.">
    ...
    <Card v-else-if="rateSheet" class="w-full max-w-2xl mx-auto mt-10">
      <CardHeader>
      ...
      <CardContent>
        <div class="flex">
          <div class="flex-1 min-w-0 space-y-4 text-sm text-muted-foreground">
            <div><strong>Description:</strong> {{ rateSheet.description }}</div>
            <div><strong>Hourly Rate:</strong> {{ rateSheet.hourlyRate }}</div>
            <div><strong>Overnight Rate:</strong> {{ rateSheet.overnightRate }}</div>
            <div><strong>Weekend Rate:</strong> {{ rateSheet.weekendRate }}</div>
            <div><strong>Effective From:</strong> {{ rateSheet.effectiveFrom }}</div>
            <div><strong>Effective To:</strong> {{ rateSheet.effectiveTo }}</div>
          </div>
          <div class="ml-6 flex-shrink-0 w-40 space-y-4">
            <span v-html="getStatus(rateSheet.status)"></span>
            <div class="border-t pt-4 mt-4">
              <dt class="text-sm font-medium text-muted-foreground">Created at</dt>
              <dd class="mt-1 text-sm" v-html="ukFormat(rateSheet.created_at)"></dd>
            </div>

            <div class="border-t pt-4">
              <dt class="text-sm font-medium text-muted-foreground">Updated at</dt>
              <dd class="mt-1 text-sm" v-html="ukFormat(rateSheet.updated_at)"></dd>
            </div>

            <div class="border-t pt-4">
              <dt class="text-sm font-medium text-muted-foreground">Rate Sheet ID</dt>
              <dd class="mt-1 text-sm font-mono">{{ rateSheet.id }}</dd>
            </div>
          </div>
        </div>
        <dl class="space-y-4">
          <div class="flex items-center gap-3 pt-4">
            <RouterLink :to="`/dashboard/rate-sheet/edit/${rateSheet.id}`">

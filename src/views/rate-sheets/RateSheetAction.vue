...
const store = useRateSheetsStore()
const { confirm } = useConfirmDialog()

const handEditAction = (id) => {
  const url = `/dashboard/rate-sheet/edit/${id}`
  router.push(url)
}

const handDeleteAction = async (id) => {
  const confirmed = await confirm('Are you sure you want to move this rate sheet to trash?', {
    confirmLabel: 'Yes, move',
    cancelLabel: 'No',
  })
  if (confirmed) {
    await store.deleteRateSheet(id)
  }
}

const handViewAction = (id) => {
  const url = `/dashboard/rate-sheet/show/${id}`
  router.push(url)
}
...
<template>
  <DropdownMenu>
    ...
    <DropdownMenuContent class="w-45 rounded">
      <DropdownMenuLabel class="text-xs opacity-75">Rate Sheet Action</DropdownMenuLabel>

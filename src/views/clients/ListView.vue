...
if (pageParam) {
  clientStore.page = Number(pageParam)
} else {
  clientStore.page = 1
}
...
const { paginatedClients, page, itemsPerPage, totalClients, buikLoading, deletingIds } =
  storeToRefs(clientStore)

async function onApply() {
  await clientStore.handleBuilAction()
}
</script>

<template>
  <div>
    <PageTitle title="Client List" subtitle="View and manage all clients.">
      <RouterLink to="/dashboard/client/add">
        <Button class="button-primary-outline">Add New Client</Button>
      </RouterLink>
    </PageTitle>
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead class="min-w-40 w-40">Name</TableHead>
            <TableHead class="min-w-50 w-50">Email</TableHead>
            <TableHead class="min-w-35 w-35">Phone</TableHead>
            <TableHead class="min-w-35 w-35">Company</TableHead>

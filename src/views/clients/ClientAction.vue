...
const handDeleteAction = async (id) => {
  const confirmed = await confirm('Are you sure you want to move this client to trash?', {
...
const handViewAction = (id) => {
  const url = `/dashboard/client/show/${id}`
  router.push(url)
}
...
<template>
  <DropdownMenu>
    ...
    <DropdownMenuContent class="w-45 rounded">
      <DropdownMenuLabel class="text-xs opacity-75">Client Action</DropdownMenuLabel>
